...
const Electronics = () => {
  ...
  const filteredTech = useMemo(() => {
    return allElectronics.filter(item => 
      item.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [searchQuery, allElectronics]);
  ...
  const currentTech = filteredTech.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredTech.length / itemsPerPage);

  const handleWhatsApp = (productName) => {
    const message = encodeURIComponent(`Hello TrendyTech, I am interested in purchasing the ${productName}. Is it currently in stock?`);
  ...
  return (
    <div className="min-h-screen bg-black text-white relative">
      ...
      {/* Main Page Content Wrapper - Hides when a card is active */}
      <div className={`transition-all duration-500 ${activeCard ? "opacity-0 pointer-events-none scale-95" : "opacity-100"}`}>
        ...
        <div className="pt-44 pb-20 px-6 md:px-16 relative z-10">
          ...
          <header className="flex flex-col items-center text-center max-w-5xl mx-auto mb-16">
            <h1 className="text-4xl md:text-7xl font-black uppercase tracking-tighter mb-6 leading-none">
              Electronics <span className="text-yellow-400">Excellence Center</span>
            </h1>
            <p className="text-gray-400 text-base md:text-lg max-w-2xl leading-relaxed border-t border-yellow-400/30 pt-6">
              Upgrade your lifestyle with our curated collection of {allElectronics.length} high-end electronic devices. From computing power to audio precision, find the tech that moves you.
            </p>
          ...
          {currentTech.length > 0 ? (
            ...
            <div className="text-center py-20 italic text-zinc-600 tracking-widest">
              No electronic items found matching your search.
            </div>
          )}

          {/* Pagination Controls */}
          {filteredTech.length > itemsPerPage && (
        ...
        {/* Footer inside the wrapper so it also hides */}
        <Footer />
      </div>
          ...
          <div className="relative bg-[#0d0d0d] border border-white/10 w-full max-w-md md:max-w-4xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="w-full md:w-1/2 p-6 md:p-12 overflow-y-auto">
              <div className="flex flex-col h-full text-left">
                <span className="text-yellow-400 text-[10px] md:text-xs font-bold uppercase tracking-[0.3em] mb-2 block">Tech & Innovation</span>
